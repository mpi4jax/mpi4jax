...
cpdef inline unicode get_error_name(hipError_t ierr):
    return py_string(hipGetErrorName(ierr))


cpdef inline unicode get_error_string(hipError_t ierr):
    return py_string(hipGetErrorString(ierr))
...
cdef inline hipError_t checked_hip_memcpy(void* dst, void* src, size_t count,
                                            hipMemcpyKind kind, MPI_Comm comm) nogil:
    cdef hipError_t ierr
    ierr = hipMemcpy(dst, src, count, kind)
    ...
    if ierr != hipSuccess:
        with gil:
            ...
            message = (
                f"hipMemcpy failed with the following error:\n"
...
cdef inline hipError_t checked_hip_stream_synchronize(
    hipStream_t stream, MPI_Comm comm
) nogil:
    cdef hipError_t ierr
    ...
    if ierr != hipSuccess:
        with gil:
            ...
            err_des = get_error_string(ierr)
            message = (
                f"hipStreamSynchronize failed with the following error:\n"
                f"\tError {ierr} {err_str}: {err_des}"
            )
...
cdef void mpi_allgather_gpu(hipStream_t stream, void** buffers,
    ...
    checked_hip_stream_synchronize(stream, comm)
    ...
    if COPY_TO_HOST:
        ...
        checked_hip_memcpy(in_buf, data, sendbytes, hipMemcpyDeviceToHost, comm)
    ...
    if COPY_TO_HOST:
        # copy back to device
        checked_hip_memcpy(out_data, out_buf, recvbytes, hipMemcpyHostToDevice, comm)
        free(in_buf)
        free(out_buf)
...
cdef void mpi_allreduce_gpu(hipStream_t stream, void** buffers,
    ...
    if COPY_TO_HOST:
        ...
        checked_hip_memcpy(in_buf, data, count, hipMemcpyDeviceToHost, comm)

    mpi_xla_bridge.mpi_allreduce(in_buf, out_buf, nitems, dtype, op, comm)

    if COPY_TO_HOST:
        # copy back to device
        checked_hip_memcpy(out_data, out_buf, count, hipMemcpyHostToDevice, comm)
        free(in_buf)
        free(out_buf)
...
cdef void mpi_bcast_gpu(hipStream_t stream, void** buffers,
    ...
    if COPY_TO_HOST:
        ...
        if rank == root:
            checked_hip_memcpy(buf, data, count, hipMemcpyDeviceToHost, comm)
    else:
        if rank == root:
            buf = data
    ...
    if COPY_TO_HOST:
        if rank != root:
            # copy back to device
            checked_hip_memcpy(out_data, buf, count, hipMemcpyHostToDevice, comm)
        free(buf)
...
cdef void mpi_gather_gpu(hipStream_t stream, void** buffers,
    ...
    if COPY_TO_HOST:
        if rank == root:
            # copy back to device
            checked_hip_memcpy(
                out_data, out_buf, recvbytes, hipMemcpyHostToDevice, comm
            )
...
cdef void mpi_recv_gpu(hipStream_t stream, void** buffers,
    ...
    if COPY_TO_HOST:
        checked_hip_memcpy(out_buf, recvbuf, count, hipMemcpyHostToDevice, comm)
        free(recvbuf)
...
cdef void mpi_send_gpu(hipStream_t stream, void** buffers,
    ...
    if COPY_TO_HOST:
        ...
        checked_hip_memcpy(sendbuf, data, count, hipMemcpyDeviceToHost, comm)
...
cdef void mpi_sendrecv_gpu(hipStream_t stream, void** buffers,
    ...
    if COPY_TO_HOST:
        ...
        checked_hip_memcpy(sendbuf, in_buf, bytes_send, hipMemcpyDeviceToHost, comm)
    ...
    if COPY_TO_HOST:
        checked_hip_memcpy(out_buf, recvbuf, bytes_recv, hipMemcpyHostToDevice, comm)
        free(recvbuf)
