...
def mpi_bcast_xla_encode_cpu_ffi(ctx, x, root, comm):
...
def mpi_bcast_xla_encode_cpu_legacy(ctx, x, root, comm):
    comm = unpack_hashable(comm)
...
def mpi_bcast_xla_encode_cpu(ctx, x, root, comm):
    ...
    if use_ffi and _has_ffi_support():
        return mpi_bcast_xla_encode_cpu_ffi(ctx, x, root, comm)
    else:
        return mpi_bcast_xla_encode_cpu_legacy(ctx, x, root, comm)
